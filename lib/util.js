...
const util = {
  ...
  buildMuon: (options = config.defaultOptions) => {
    ...
    const extra_args_file = path.join(config.rootDir, 'args.gni')
    let extra_args = ''
    if (fs.existsSync(extra_args_file)) {
      extra_args = 'import(\\"' + extra_args_file + '\\")'
    }
    util.run('gn', ['gen', config.outputDir, '--args="' + args + extra_args + '"'], options)
